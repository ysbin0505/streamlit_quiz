    ...
    for prev_week in range(1, week_num):
        ...
        for f in os.listdir(prev_dir):
            if f.endswith(".json"):
                key = strip_prefix(f)
                used_keys.add(key)
    # 3. 이번 주차에서 사용할 N개 선정 (이전 주차 사용분 제외)
    remain_keys = [k for k in candidate_keys if k not in used_keys]
    # remain_keys = random.sample(remain_keys, min(files_per_week, len(remain_keys)))  # 랜덤 추출 시 사용
    remain_keys = remain_keys[:files_per_week]  # 이름순(혹은 순서)으로 N개
